...
class MLP(object):
    ...
    def make_network_params(self):
        # in_size: 2*h, hid_size: [], out_size: 1
        # dims: [in_size, hid_size1, hid_size2, ..., hid_sizeN, out_size]
        # actual: [2*h, 1]
        dims = [self.in_size] + self.hid_sizes + [self.out_size]
        # weight_sizes: [in_size, hid_size1], [hid_size1, hid_size2], ..., [hid_sizeN, out_size]
        # actual: [2*h, 1]
        weight_sizes = list(zip(dims[:-1], dims[1:]))
        # weights: [in_size, hid_size1], [hid_size1, hid_size2], ..., [hid_sizeN, out_size]
        # actual: [2*h, 1]
        weights = [tf.Variable(self.init_weights(s), name='MLP_W_layer%i' % i)
                   for (i, s) in enumerate(weight_sizes)]
        # biases: [hid_size1], [hid_size2], ..., [out_size]
        # actual: [1]
