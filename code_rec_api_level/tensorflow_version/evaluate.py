...
class ProgModel(object):
    ...
    def __init__(self, args, training_file_count, valid_file_count):
        ...
        with self.graph.as_default():
            ...
            if restore_file is not None:
                ...
                self.restore_model2('D:\\aiserver\\sources\\models\\'+restore_file)
        ...
        for (internal_id, task_id) in enumerate(self.params['task_ids']):
            with tf.variable_scope("out_layer_task%i" % task_id):
                ...
                with tf.variable_scope("softmax"):
                    ...
                    self.weights['softmax_weights'] = tf.Variable(glorot_init([self.params['softmax_size'],22821]))
                    self.weights['softmax_biases'] = tf.Variable(np.zeros([22821]).astype(np.float32))
                ...
                top_5_accuracy = tf.nn.in_top_k(computed_values, groundtruth, k=5)
                self.ops['top_5_accuracy_task%i' % task_id] = top_5_accuracy
    ...
    def run_epoch(self, epoch_name: str, data, is_training: bool):
        ...
        top_5_accuracy_ops = [self.ops['top_5_accuracy_task%i' % task_id] for task_id in self.params['task_ids']]
        ...
        while count < file_count:
            ...
            for step, batch_data in enumerate(batch_iterator):
                ...
                if is_training:
                    ...
                    fetch_list = [self.ops['loss'], accuracy_ops, top_5_accuracy_ops, self.ops['train_step']]
                else:
                    batch_data[self.placeholders['out_layer_dropout_keep_prob']] = 1.0
                    fetch_list = [self.ops['loss'], accuracy_ops, top_5_accuracy_ops]

                #print(batch_data)

                result = self.sess.run(fetch_list, feed_dict=batch_data)
                (batch_loss, batch_accuracies, batch_top_5_accuracies) = (result[0], result[1], result[2])
                loss += batch_loss * num_graphs
                accuracies.append(np.array(batch_accuracies) * num_graphs)
                top_5_accuracies.append(np.array(batch_top_5_accuracies) * num_graphs)
                tempAcc.append(np.array(batch_accuracies) * num_graphs)
                tempTop5Acc.append(np.array(batch_top_5_accuracies) * num_graphs)

                print("Running %s, %s file %i, batch %i (has %i graphs). Loss so far: %.4f" % (epoch_name, filestr,count, total,
        ...
        top_5_accuracies = np.sum(top_5_accuracies, axis=0) / processed_graphs
        ...
        return loss, accuracies, top_5_accuracies, instance_per_sec, total, processed_graphs,time_str, read_data_time_str
    ...
    def train(self):
        ...
        with self.graph.as_default():
            if self.args.restore is not None:
                valid_loss, valid_accs, valid_speed, valid_batch, valid_total_graph,valid_time,valid_read_data_time = self.run_epoch("Resumed (validation)", self.mini_valid_data, False)
            ...
            for epoch in range(1, self.params['num_epochs'] + 1):
                print("== Epoch %i" % epoch)
                train_loss, train_accs, train_speed, train_batch, train_total_graph, train_time,train_read_data_time= self.run_epoch("epoch %i (training)" % epoch,
                ...
                print("\r\x1b[K Train: loss: %.5f | acc: %s | instances/sec: %.2f | train_batch: %i | train_total_graph: %i | train_time: %s | train_read_data_time: %s" % (train_loss,
                                                                                                                                                                            accs_str,
                                                                                                                                                                            train_speed, train_batch, train_total_graph,train_time,train_read_data_time))
                valid_loss, valid_accs, valid_speed, valid_batch, valid_total_graph,valid_time,valid_read_data_time= self.run_epoch("epoch %i (validation)" % epoch,
                ...
                print("\r\x1b[K Valid: loss: %.5f | acc: %s | instances/sec: %.2f | valid_batch: %i | valid_total_graph: %i | valid_time: %s | valid_read_data_time: %s" % (valid_loss,
                                                                                                                                                                            accs_str,
                ...
                if val_acc > best_val_acc:
                    ...
                    self.save_model2('D:\\aiserver\\sources\\models\\')
...
class DenseGGNNProgModel(ProgModel):
    def __init__(self, args, training_file_count,valid_file_count):
        ...
        with open('D:\\aiserver\\sources\\AndroidVocabulary\\APIVocabulary.txt','r') as f:
            self.api2idx = {}
            self.idx2api = {}
        ...
        with open('D:\\aiserver\\sources\\AndroidVocabulary\\WholeVocabulary.txt','r') as f:
            self.word2idx = {}
            self.idx2word = {}
        ...
        self.variable2index = {}
        self.index2variable = {}
        with open('D:\\aiserver\\sources\\AndroidVocabulary\\VariableNameVocabulary.txt','r') as f:
            lines = f.readlines()
    ...
    def prepare_specific_graph_model(self) -> None:
        ...
        self.weights['index2vector'] = tf.Variable(dtype=tf.float32,initial_value=np.random.uniform(-0.5, 0.5,[21505, e_dim]))

        # 这里加入变量名模型
        self.placeholders['variable_orders'] = tf.placeholder(tf.int32, [None, 10,], name='variable_order')
        self.weights['variable_index2vector'] = tf.Variable(dtype=tf.float32, initial_value=np.random.uniform(-0.5, 0.5,[7142, v_dim]))

        # 对变量名和api进行embeddding,变量名维度也是[b, v, v_dim]
        self.placeholders['variable_orders_embed_first'] = tf.nn.embedding_lookup(self.weights['variable_index2vector'],self.placeholders['variable_orders'])
        self.placeholders['variable_orders_embed'] = tf.layers.dense(self.placeholders['variable_orders_embed_first'], self.params['variable_embed_size'],tf.tanh)
        ...
        self.placeholders['variable_orders_embed'] = tf.layers.dense(self.placeholders['variable_orders_embed'], self.params['variable_embed_size'],tf.tanh)
        self.placeholders['variable_orders_embed'] = tf.nn.dropout(self.placeholders['variable_orders_embed'],self.placeholders['out_layer_dropout_keep_prob'])
        self.placeholders['variable_orders_embed'] = tf.layers.dense(self.placeholders['variable_orders_embed'], self.params['variable_embed_size'],tf.tanh)
        self.placeholders['variable_orders_embed'] = tf.nn.dropout(self.placeholders['variable_orders_embed'],self.placeholders['out_layer_dropout_keep_prob'])
        ...
        self.placeholders['orders_embed'] = tf.nn.embedding_lookup(self.weights['index2vector'],self.placeholders['input_orders'])
        ...
        # 使用mask过滤掉0的部分
        self.placeholders['variable_mask'] = tf.placeholder(tf.float32, [None, 10, self.params['variable_embed_size']], name='variable_mask')
        self.placeholders['variable_orders_embed'] = self.placeholders['variable_orders_embed'] * self.placeholders['variable_mask']
        ...
        self.placeholders['initial_node_representation'] = tf.placeholder(tf.float32,
                                                                          ...
                                                                          name='node_features')
        # [b, v]
        self.placeholders['node_mask'] = tf.placeholder(tf.float32, [None, None, self.params['hidden_size']], name='node_mask')
        ...
        if self.params['use_edge_bias']:
            self.weights['edge_biases'] = tf.Variable(np.zeros([self.num_edge_types, 1, h_dim]).astype(np.float32),name='edge_biases')
        ...
        for d in raw_data:
            ...
            bucketed[chosen_bucket_idx].append({
                ...
                'mask': [[1.0 for _ in range(self.params['hidden_size'])] for _ in range(n_active_nodes)] +
                        [[0. for _ in range(self.params['hidden_size'])] for _ in range(chosen_bucket_size - n_active_nodes)],
    ...
    def make_batch(self, elements):
        batch_data = {'adj_mat':[], 'orders':[], 'labels':[], 'node_mask':[], 'task_masks':[], 'variables':[], 'variable_masks':[]}
        # 逐数据将信息填入
        ...
        for d in elements:
            ...
            #print(d['variable'])
            variables = [idx for idx in d['variable'][0][:10]] + [7141 for _ in range(max_variable_size - variable_length)]
    ...
    def example_evaluation(self):
        ...
        example_molecules, _, _ = self.process_raw_graphs(example_molecules,is_training_data=False,bucket_sizes=np.array([29]))
    ...
    def predict(self, graphRepresent, graphVocab, variableName, single_targets=[[-1]]):
        ...
        json_now = {'graph':json.loads(graphRepresent), 'orders':[cur_order], 'targets':single_targets, 'variable':cur_indexes}
    ...
    def test(self):
        with self.graph.as_default():
            if self.args.restore is not None:
                test_loss, test_accs, test_top_5_accs, test_speed, test_batch, test_total_graph, test_time, test_read_data_time = self.run_epoch(
                    "Resumed (Test)", self.mini_valid_data, is_training=False)
                accs_str = " ".join(["%i:%.5f" % (id, acc) for (id, acc) in zip(self.params['task_ids'], test_accs)])
                top5_accs_str = " ".join(["%i:%.5f" % (id, acc) for (id, acc) in zip(self.params['task_ids'], test_top_5_accs)])
                print("\r\x1b[K Test: loss: %.5f | top1 acc: %s | top5 acc: %s | instances/sec: %.2f | test_batch: %i | test_total_graph: "
                    "%i | test_time: %s | test_read_data_time: %s" % (test_loss, accs_str, top5_accs_str, test_speed, test_batch,
                                                                      test_total_graph, test_time, test_read_data_time))


Parser = namedtuple('parser',['data_dir','config_file','config','restore','restrict','freeze_graph_model'])
parser = Parser(data_dir='',config_file=None,config=None,restore="model_best-2020-09-25-09-34-04_14246",restrict=None,freeze_graph_model=False)
